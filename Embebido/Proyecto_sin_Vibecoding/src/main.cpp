...
#define WIFI_CANAL 6 // canal fijo => asociacion rapida en Wokwi
...
void puerta_deteccion(void *pvParametros)
{
  ...
  while (1)
  {
    ...
    {
      finishStats(); // caso B: cierra el muestreo de métricas por serial
    }
...
void setup()
{
  ...
  // Métricas de CPU/memoria: arranca el muestreo. Para cerrar y ver promedios:
  //   - esperar 10 s sin interacción (caso A, abajo en loop()),
  //   - escribir 'M' por el monitor serial, o
  //   - publicar en pawgate/pawgate-001/cmd/metrics (caso B, por MQTT).
  initStats();
}

void loop()
{
  // Caso A "10 s sin interacción": a los 10 s cierra el muestreo e imprime los
  // promedios de CPU/memoria automáticamente (igual que el ejemplo de la cátedra).
...
void callback(char *topico, byte *message, unsigned int length)
{
  ...
  else if (strcmp(comando + 1, "metrics") == 0)
  {
    Serial.println("FINISH STATS DESDE MQTT");
    finishStats(); // caso B: cierra el muestreo al recibir un mensaje MQTT
    return;        // no cambia el estado de la puerta
  }
  else
  {
